...
async function main() {
  ...
  // Only rewrite the file when the badge list actually changed, so no-op
  // runs produce no git diff (and the timestamp tracks the last real change).
  const prevList = (await previousBadges()).map((b) => ({
    name: b.name, issuer: b.issuer, image: b.image, link: b.link,
  }));
  if (JSON.stringify(prevList) === JSON.stringify(list)) {
    console.log(`\nNo change. ${list.length} synced badge(s), ${downloaded} new image(s).`);
    return;
  }
